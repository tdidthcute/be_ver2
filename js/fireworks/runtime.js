...
function applyResolvedBackground() {
	...
	if (resolvedBackground.source === "user") {
		backgroundManager.applyBackground(resolvedBackground.background).then((result) => {
			if (result.ok) {
				backgroundManager.setStatus("Đang dùng nền bạn đã đặt", "success");
				return;
			}
			...
			if (!fallbackBackground.value) {
				backgroundManager.clearBackground();
				backgroundManager.setStatus("Nền bạn đặt không hợp lệ, hiện chưa có nền mặc định", "error");
				return;
			}

			backgroundManager.applyBackground(fallbackBackground).then((fallbackResult) => {
				if (fallbackResult.ok) {
					backgroundManager.setStatus("Nền bạn đặt không hợp lệ, đã quay về nền mặc định", "idle");
					return;
				}

				backgroundManager.clearBackground();
				backgroundManager.setStatus("Cả nền bạn đặt và nền mặc định đều không hợp lệ", "error");
			});
		});
		return;
	}
	...
	if (resolvedBackground.source === "default") {
		backgroundManager.applyBackground(resolvedBackground.background).then((result) => {
			if (result.ok) {
				backgroundManager.setStatus("Đang dùng nền mặc định", "idle");
				return;
			}

			backgroundManager.clearBackground();
			backgroundManager.setStatus("Nền mặc định không hợp lệ, hiện không hiển thị nền", "error");
		});
		return;
	}
