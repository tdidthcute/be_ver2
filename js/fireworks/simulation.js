    ...
    for (let index = stars.length - 1; index >= 0; index -= 1) {
      ...
      if (star.sparkFreq) {
        ...
        while (star.sparkTimer < 0) {
          star.sparkTimer +=
            star.sparkFreq * 0.75 + star.sparkFreq * inverseBurnRate * 4;
          Spark.add(
            star.x,
            star.y,
            star.sparkColor,
...
function getWordDots(word) {
  ...
  const fontSize = MyMath.randomInt(
    appConfig.wordFontSizeMin,
    appConfig.wordFontSizeMax,
  );
  ...
  if (!wordDotCache.has(cacheKey)) {
    wordDotCache.set(
      ...
      MyMath.literalLattice(
        word,
        appConfig.wordPointDensity,
        appConfig.wordFontFamily,
        `${fontSize}px`,
      ),
    );
  }
...
class Shell {
  ...
  burst(x, y) {
    ...
    if (typeof this.color === "string") {
      ...
      throw new Error(`Cấu hình màu pháo hoa không hợp lệ: ${this.color}`);
    }
